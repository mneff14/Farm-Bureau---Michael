...
read_in_df <- function(location) {
  ...
  # Return a list of strings that's the names of all PDFs downloaded
  return(df)
}
...
scrape_all_pdf_dates_and_elements <- function(url) {
  ...
  # Use a lapply to extract the Dates and assign them to the second column
  df$V2 <- Reduce(c, lapply(df$V2, ymd()))
  
  
  # Get rid of any rows that were not filled
  # df <- na.omit(df)
...
extract_date <- function(xml_element) {
  ...
  # Extract the year from the href
  month <- tryCatch(
    expr = stringr::str_extract(href_string, "(?<=mprpt)\\d\\d"), # Two digits preceeded by "mprpt"
    error = function(cnd) {
      # Returns NULL if an error is produced
  ...
  # Extract the year from the href
  day <- tryCatch(
    expr = stringr::str_extract(href_string, "(?<=mprpt\\d\\d)\\d\\d"), # Two digits preceeded by the month
    error = function(cnd) {
      # Returns NULL if an error is produced
  ...
  # print(paste0("Before: ", href_date,  " ", is.Date(href_date)))
...
pdf_urls <- get_pdf_urls(url = "https://barley.idaho.gov/archived_reports.html",
                         folder_path = "C:\\Users\\mneff\\Documents\\Git\\Farm_Bureau_F19\\Personal\\Michael\\PDF URLs",
                         return_df = TRUE)


# folder_path <- "C:\\Users\\mneff\\Documents\\Git\\Farm_Bureau_F19\\Personal\\Michael\\PDF URLs"3
pdf_name <- paste0("C:/Users/mneff/Documents/Git/Farm_Bureau_F19/Personal/Michael/PDF URLs/market_report", 1, ".pdf")

download.file(url = pdf_urls[1,1], destfile = pdf_name, 
